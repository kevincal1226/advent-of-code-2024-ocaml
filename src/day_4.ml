...
let rec read_input acc =
    try
    (read_line ()
                |> string_to_list |> Array.of_list) :: acc
            |> read_input
...
let rec find_xmas (grid: char array array) (idx: int) (row: int) (col: int) (changes: int array) =
    if idx = 4 then
        1
    else if row < 0 || col < 0 || row >= num_rows || col >= num_cols then
        0
    else if grid.(row).(col) != xmas.(idx) then
        0
    else
...
let find_mas_in_x (grid: char array array) (row: int) (col: int) =
    if row = 0 || col = 0 || row = num_rows-1 || col = num_cols-1 || grid.(row).(col) != 'A' then
        0
    else
...
input
|> Array.mapi (fun row_idx row  -> row
        |> Array.mapi (fun col_idx c -> col_idx
