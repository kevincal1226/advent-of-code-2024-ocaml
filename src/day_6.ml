...
let rec read_input curr_row start_row start_col grid =
  try
    let line = read_line () |> String.to_seq |> Array.of_seq in
  ...
  with
  | End_of_file -> start_row, start_col, grid |> List.rev |> Array.of_list
;;
...
|> Seq.map (fun (row, col) ->
  ...
  | '.' ->
    let ret =
      grid.(row).(col) <- '#';
      let cycle = find_cycle start_row start_col 0 dirs grid (Hashtbl.create 1) in
      grid.(row).(col) <- '.';
      cycle
    in
    ret
