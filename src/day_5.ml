...
let insert_pair tbl key value =
  ...
  | None ->
    ...
    Hashtbl.add temp value 1;
    Hashtbl.add tbl key temp
...
let rec check_valid_update orders l r update =
  if l = Array.length update
  then update.(Array.length update / 2)
  else if r = Array.length update
  then check_valid_update orders (l + 1) (l + 2) update
  else (
    match Hashtbl.find_opt orders update.(r) with
    | None -> check_valid_update orders l (r + 1) update
    | Some tbl ->
      (match Hashtbl.find_opt tbl update.(l) with
       | None -> check_valid_update orders l (r + 1) update
       | Some _ -> 0))
;;
...
let rec fix_update orders l r fixed update =
  if l = Array.length update
  then if fixed then update.(Array.length update / 2) else 0
  else if r = Array.length update
  then fix_update orders (l + 1) (l + 2) fixed update
  else (
    match Hashtbl.find_opt orders update.(r) with
